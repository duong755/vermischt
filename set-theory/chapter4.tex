...
\begin{theorem}[Nguyên lý quy nạp toán học (trường hợp cơ sở là một số nguyên)]
...
\begin{theorem}[Nguyên lý giảm vô hạn]
\end{theorem}
...
Tương tự với ``dạng số nguyên'' của nguyên lý quy nạp toán học, chúng ta có hệ quả sau. Hệ quả này cho phép chúng ta áp dụng nguyên lý thứ tự tốt linh hoạt hơn.
...
\begin{theorem}\label{theorem:linear-combination-and-common-divisor}
	Nếu số nguyên $d$ là ước chung của hai số nguyên $a$ và $b$ thì với mọi số nguyên $x, y$, $d$ là ước của $a\cdot x + b\cdot y$. (Chúng ta gọi số nguyên viết được dưới dạng $a\cdot x + b\cdot y$ là một \textbf{tổ hợp tuyến tính} của $a$ và $b$)
\end{theorem}
...
\begin{proof}
	Theo Định lý~\ref{theorem:bound-of-divisors}, nếu số nguyên $d$ là ước chung của hai số nguyên $a, b$ thì $\abs{d}$ không vượt quá số nhỏ nhất và khác $0$ trong hai số $\abs{a}$ và $\abs{b}$.

	Kí hiệu số nhỏ nhất và khác $0$ trong hai số $\abs{a}$ và $\abs{b}$ là $c$. Bằng kí hiệu, phát biểu trên có nghĩa là: Nếu số nguyên $d$ là ước chung của hai số nguyên $a, b$ thì $\abs{d}\leq c$. Mà $-\abs{d}\leq d \leq \abs{d}$ nên $-c\leq d\leq c$. Theo nguyên lý thứ tự tốt, tập hợp các ước chung của $a$ và $b$ có một số nguyên lớn nhất.
\end{proof}
...
Qua chứng minh trên, cùng với việc $1$ là ước của mọi số nguyên, chúng ta rút ra hệ quả: ước chung lớn nhất của hai số nguyên (mà hai số nguyên đó không đồng thời bằng $0$) là một số nguyên dương. Trên đây, chúng ta đã sử dụng nguyên lý thứ tự tốt để chứng minh sự tồn tại của ước chung lớn nhất. Tuy nhiên, với cách này, chúng ta không có thêm thông tin gì về ước chung lớn nhất, cũng như cách tìm ước chung lớn nhất. Thuật toán Euclid mở rộng dưới đây cung cấp một cách tìm ước chung lớn nhất hiệu quả.

\begin{theorem}[Thuật toán Euclid mở rộng]
	Cho hai số nguyên $a$ và $b$, trong đó $b\ne 0$. Áp dụng thuật toán chia Euclid để thu được thương $q_{0}$ và dư $r_{0}$: $a = b\cdot q_{0} + r_{0}$.
...
\begin{proof}[Chứng minh thuật toán Euclid mở rộng]
	\begin{enumerate}[label={(\roman*)}]
		...
		\item Nếu $r_{0}\ne 0$ thì thuật toán Euclid được áp dụng ít nhất hai lần.
...
\begin{theorem}[Đồng nhất thức B\'{e}zout]
	Nếu $d$ là ước chung lớn nhất của hai số nguyên $a$ và $b$ thì tồn tại các số nguyên $x$ và $y$ sao cho $d = a\cdot x + b\cdot y$. Nói cách khác, ước chung lớn nhất của hai số nguyên là một tổ hợp tuyến tính của hai số nguyên đó.
\end{theorem}
...
\begin{proof}[Chứng minh đồng nhất thức B\'{e}zout với nguyên lý thứ tự tốt]
	...
	$a$ và $b$ không đồng thời bằng $0$. Không mất tính tổng quát, chúng ta giả sử $b\ne 0$. Khi đó $a\cdot 0 + b\cdot b = b^{2} > 0$ là một phần tử của $S$. Do đó tập hợp $S$ khác rỗng. $S$ là tập hợp con của tập hợp các số nguyên dương nên theo nguyên lý thứ tự tốt, $S$ có phần tử nhỏ nhất. Kí hiệu phần tử nhỏ nhất của $S$ là $d$. Như vậy tồn tại các số nguyên $x, y$ sao cho $d = a\cdot x + b\cdot y$. Chúng ta chứng minh rằng $d$ chính là ước chung lớn nhất của $a$ và $b$.

	Giả sử $c$ là ước chung của $a$ và $b$. Theo Định lý~\ref{theorem:linear-combination-and-common-divisor}, $c$ là ước của $a\cdot x + b\cdot y = d$. Theo Định lý~\ref{theorem:bound-of-divisors}, $\abs{c}\leq d$. Mà $c\leq \abs{c}$ nên theo tính chất bắc cầu, $c\leq d$. Do đó $d$ là ước chung lớn nhất của $a$ và $b$.
\end{proof}
...
\begin{proof}
	...
	Nếu $c = 0$ thì $a\cdot d = d\cdot x = 0$. Còn nếu $c\ne 0$ thì $c\cdot d\ne 0$, kéo theo $a\cdot y - b\cdot x = 0$. Do đó quan hệ $\sim$ trên tập hợp $\mathbb{Z}\times(\mathbb{Z} - \{0\})$ có tính chất bắc cầu.
