...
\begin{exercise}
	...
	\end{enumerate}
\end{exercise}\begin{exercise}[Nguyên lý Cantor về các khoảng đóng lồng nhau]
	Với mỗi số nguyên dương $n$, $I_{n} = [a_{n}, b_{n}]$. Giả sử với mỗi số nguyên dương $n$, có $I_{n} \supseteq I_{n+1}$.
	\begin{enumerate}[label={(\roman*)}]
		\item Chứng minh rằng tập hợp các số thực $a_{n}$ có cận trên nhỏ nhất (chúng ta sẽ kí hiệu là $a$), và tập hợp các số thực $b_{n}$ có cận dưới lớn nhất (chúng ta sẽ kí hiệu là $b$).
		\item Chứng minh rằng giao của tất cả các khoảng đóng $I_{n}$ khác rỗng.
...
Với dạng lượng giác của số phức, việc thực hiện phép nhân, và lũy thừa trở nên dễ dàng bằng định lý sau.
\begin{theorem}
	\begin{enumerate}[label={(\roman*)}]
...
Phép nhân một số phức $z_{0}$ với số phức $z\ne 0$ tương ứng với một phép vị tự-quay (Xem Hình~\ref{fig:complex-multiplication})

\begin{figure}[htp]
	\centering
	\begin{tikzpicture}[>=Stealth,scale=2.0]
		\draw[help lines, color=gray!30, dashed,step=0.5cm] (-1.5,-1.5) grid (2.0,2.0);
...
Biểu diễn hình học của các căn bậc $n$ của một số phức khác không cho trước là $n$ điểm. $n$ điểm này thuộc đường tròn đơn vị và là $n$ đỉnh của một $n$-giác đều với tâm là gốc tọa độ (Xem Hình~\ref{fig:5throot-of-unity}.)
