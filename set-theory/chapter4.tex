...
\begin{definition}[Tính chia hết]
	\begin{enumerate}[label={(\roman*)}]
		\item Số tự nhiên $a$ được gọi là \textit{chia hết} số tự nhiên $b$ nếu tồn tại số tự nhiên $k$ sao cho $k\cdot a = b$. Chúng ta còn nói số tự nhiên $b$ \textit{chia hết cho} số tự nhiên $a$, hay $a$ là ước của $b$, $b$ là bội của $a$, và kí hiệu là $a\mid b$.
		\item Số nguyên $a$ được gọi là \textit{chia hết} số nguyên $b$ nếu tồn tại số nguyên $k$ sao cho $k\cdot a = b$. Chúng ta còn nói số nguyên $b$ \textit{chia hết cho} số nguyên $a$, hay $a$ là ước của $b$, $b$ là bội của $a$ và kí hiệu là $a\mid b$.
	\end{enumerate}
\end{definition}

Từ định nghĩa tính chia hết và các tính chất của số tự nhiên và số nguyên, chúng ta rút ra các nhận xét sau.
\begin{itemize}
	\item Số $0$ chia hết cho mọi số nguyên.
	\item Số $0$ không phải ước của bất cứ số nguyên khác $0$ nào.
	\item Số $1$ và $(-1)$ là ước của mọi số nguyên.
	\item Nếu số tự nhiên $a$ là ước của số tự nhiên $b$ và $a\ne 0$ thì tồn tại duy nhất số tự nhiên $k$ sao cho $k\cdot a = b$.
	\item Nếu số nguyên $a$ là ước của số nguyên $b$ và $a\ne 0$ thì tồn tại duy nhất số tự nhiên $k$ sao cho $k\cdot a = b$.
	\item Bội số tự nhiên khác $0$ nhỏ nhất của số tự nhiên $a\ne 0$ là $a$.
\end{itemize}

Nhận xét cuối cùng ngụ ý rằng nếu số tự nhiên $a$ lớn hơn $1$ thì các số tự nhiên $1, 2, \ldots, a - 1$ không chia hết cho $a$.

\begin{theorem}[Thuật toán chia Euclid]
	Cho số nguyên dương $a$ và một số nguyên $b$. Tồn tại duy nhất cặp số nguyên $q$ và $r$ sao cho $b = a\cdot q + r$ và $0\leq r < a$.
\end{theorem}

\begin{proof}
\end{proof}

Từ giờ trở đi, chúng ta chỉ sử dụng ước nguyên, bội nguyên (và dùng thuật ngữ ước, bội) trừ khi nêu khác đi.
...
\begin{corollary}
	Tập hợp con khác rỗng của tập hợp số nguyên lớn hơn hoặc bằng số nguyên $x$ nào đó có phần tử nhỏ nhất.
\end{corollary}
