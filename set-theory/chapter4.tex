...
\begin{theorem}
	Nếu $a, b, c$ là các số nguyên và $b, c$ khác $0$ thì $\frac{a}{b} = \frac{ac}{bc}$.
\end{theorem}
...
\begin{proposition}\label{proposition:same-denominator}
...
\begin{proof}
	\begin{enumerate}[label={(\roman*)}]
		...
		\item Theo định nghĩa phép cộng số hữu tỉ
			  ...
			  \end{align*}
		\item Theo Mệnh đề~\ref{proposition:same-denominator}
		      \begin{align*}
...
\begin{theorem}
	...
	\begin{enumerate}[label={(\roman*)}]
		\item Tồn tại đúng một số hữu tỉ $\theta$ sao cho với mọi số hữu tỉ $q$, có $q + \theta = \theta + q = q$.
		\item Với mỗi số hữu tỉ $q$, tồn tại đúng một số hữu tỉ $q'$ sao cho $q + q' = q' + q = \theta$.
	\end{enumerate}
\end{theorem}
...
\begin{definition}[Nghịch đảo]
\end{definition}

\begin{theorem}[Phép chia hai số hữu tỉ]
\end{theorem}
