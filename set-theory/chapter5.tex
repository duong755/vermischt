...
Kết quả chính của mục này là sự tương đương, hay đẳng cấu của các mô hình số thực. Trước khi đi đến chứng minh cho định lý đó, chúng ta cần chuẩn bị một số kết quả khác. Chúng ta định nghĩa tập hợp số thực là $\mathscr{D}_{\mathbb{Q}}$.
...
Tính trù mật của tập hợp số hữu tỉ là một kết quả cần thiết trong chứng minh ở nội dung tiếp theo. Để chứng minh tính chất này, chúng ta cần đến thuộc tính Archimedes.
...
\begin{theorem}\label{theorem:uniqueness-of-complete-ordered-field}
    Mọi mô hình số thực đều đẳng cấu với $\mathscr{D}_{\mathbb{Q}}$ (tập hợp các lát cắt Dedekind trên tập hợp số hữu tỉ, định nghĩa ở Mục~\ref{section:dedekind-cuts}). Cụ thể hơn, nếu tập hợp $R$ với phép cộng (${+}_{R}$) với phần tử đồng nhất của phép cộng ($0_{R}$), phép nhân (${\times}_{R}$) với phần tử đồng nhất của phép nhân ($1_{R}$), và quan hệ thứ tự ($\leq_{R}$) thỏa mãn hệ tiên đề về số thực thì tồn tại một song ánh $f: \mathscr{D}_{\mathbb{Q}}\to R$ sao cho
...
Bạn đọc lưu ý rằng: với Định lý~\ref{theorem:embed-Q-into-the-set-of-dedekind-cuts}, chúng ta đồng nhất mỗi số hữu tỉ $q$ với lát cắt $q^{*} = \{ x \mid x\in\mathbb{Q} \wedge x < q \}$ (tập hợp các số hữu tỉ nhỏ hơn $q$). Chứng minh dưới đây dài, tái hiện quá trình xây dựng tập hợp số thực (bằng lát cắt Dedekind) bắt đầu từ tập hợp số tự nhiên, tập hợp số nguyên, tập hợp số hữu tỉ. Do vậy chúng tôi chia chứng minh thành các bước nhỏ hơn để tiện theo dõi.
...
\begin{proof}
    ...
    \begin{enumerate}[label={\textbf{Bước \arabic*.}},itemindent=1cm]
        ...
        \item Định nghĩa ảnh của các số hữu tỉ dưới ánh xạ $f$.
              ...
              Theo chứng minh ở \textbf{Bước 4}, nếu số nguyên $n$ khác $0$ thì $f(n)\ne 0_{R}$. Do đó, nếu số nguyên $n$ khác $0$ thì $f(n)$ có nghịch đảo. Theo \textbf{Bước 3}, nếu $\frac{a}{b} = \frac{c}{d}$ thì $f(a)\times_{R} {(f(b))}^{-1} = f(c)\times_{R} {(f(d))}^{-1}$. Điều này là cơ sở để chúng ta định nghĩa ảnh của một số hữu tỉ $q = \frac{a}{b}$ dưới ánh xạ $f$ như sau
              \[
        ...
        \item Chứng minh $f(q_{1} + q_{2}) = f(q_{1}) +_{R} f(q_{2})$, $f(q_{1}q_{2}) = f(q_{1}) \times_{R} f(q_{2})$, $q_{1} < q_{2}\implies f(q_{1}) <_{R} f(q_{2})$ với mọi số hữu tỉ $q_{1}, q_{2}$.
              ...
              Do đó $f\left(\frac{a}{b}\right) <_{R} f\left(\frac{c}{d}\right)$.
        \item Định nghĩa ảnh của các số thực dưới ánh xạ $f$.

              Với mỗi số thực $x$, chúng ta kí hiệu $S_{x}$ là tập hợp tất cả các số hữu tỉ nhỏ hơn $x$. Tập hợp $S_{x}$ là một lát cắt trên tập hợp các số hữu tỉ, vì
              \begin{enumerate}[label={(DC\arabic*)},itemindent=0.5cm]
                  \item $S_{x}$ không rỗng, $\mathbb{Q}\setminus S_{x}$ không rỗng, theo định lý về phần nguyên của số thực.
              ...
              Vì $S_{x}$ có cận trên nên theo kết quả ở \textbf{Bước 6}, ảnh của $S_{x}$ dưới ánh xạ $f$ (được kí hiệu là $f[S_{x}]$) cũng có cận trên. Theo tiên đề về cận trên, $f[S_{x}]$ có cận trên nhỏ nhất. Chúng ta định nghĩa ảnh của $x$ dưới ánh xạ $f$ là $\sup f[S_{x}]$.
    \end{enumerate}
    ...
    \begin{enumerate}[label={\textbf{Bước \arabic*.}},itemindent=1cm,start=8]
        \item Chứng minh rằng với mọi số thực $x, y$, có $x\leq y$ kéo theo $f(x) \leq_{R} f(y)$.
...
\begin{theorem}[Tính đầy đủ-Dedekind]
\end{theorem}
...
\begin{theorem}[Định lý Bolzano-Weierstra{\ss}]
\end{theorem}
...
\begin{definition}[Hàm thực\index{Hàm thực}]
\end{definition}
...
\begin{theorem}[Hàm đa thức]
\end{theorem}

\begin{theorem}[Định lý giá trị trung gian]
\end{theorem}

\subsection{Hàm lũy thừa. Hàm mũ. Hàm logarithm}

\begin{theorem}[Lũy thừa nguyên của số thực]
\end{theorem}

\begin{theorem}[Căn bậc $n$]
\end{theorem}

\begin{definition}[Lũy thừa hữu tỉ của số thực không âm]
\end{definition}

\begin{definition}[Lũy thừa thực của số thực không âm]
\end{definition}
