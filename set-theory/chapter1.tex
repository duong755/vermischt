...
Từ hai mệnh đề $P$ và $Q$, người ta định nghĩa ra phép toán VÀ (hội\index{Phép hội}) và HOẶC (tuyển\index{Phép tuyển}). Hội của $P$ và $Q$ là đúng nếu cả hai mệnh đề đúng, là sai nếu ít nhất một trong hai mệnh đề sai. Tuyển của $P$ và $Q$ là đúng nếu ít nhất một trong hai mệnh đề đúng, là sai nếu cả hai mệnh đề sai. Hội của $P$ và $Q$ được kí hiệu là $P\wedge Q$. Tuyển của $P$ và $Q$ được kí hiệu là $P\vee Q$.
...
\begin{table}[htp]
	...
	\[
		...
		\end{array}
	\]
	\caption{Bảng chân trị\index{Bảng chân trị} của các mệnh đề được tạo ra từ ba phép toán phủ định ($\neg$), hội ($\wedge$) và tuyển ($\vee$).}\label{section1:truth-table-of-not-and-or}
\end{table}
...
\begin{exercise}\label{propositional-logic:exercise6}
	...
	\]
	[Gợi ý: Các biểu thức $P\wedge Q$, $\neg P\wedge Q$, $\neg P\wedge \neg Q$ tương ứng với các hàng nào trong bảng chân trị của $P\implies Q$?]
\end{exercise}
...
\begin{enumerate}[label={(\arabic*)},itemsep=0pt]
	...
	\item Chỉ có đúng một tập hợp không chứa phần tử nào. Tập hợp đó được gọi là \textbf{tập hợp rỗng\index{Tập hợp rỗng}}. Chúng ta kí hiệu tập hợp rỗng là $\varnothing$.
...
\subsection{Quan hệ bao hàm giữa các tập hợp\index{Quan hệ bao hàm giữa các tập hợp}}
...
\begin{definition}
	(Tập hợp) $A$ là tập hợp con\index{Tập hợp con} của (tập hợp) $B$ nếu và chỉ nếu mỗi phần tử của $A$ cũng là phần tử của $B$.
...
\begin{definition}[Tập hợp lũy thừa]
	\textbf{Tập hợp lũy thừa\index{Tập hợp lũy thừa}} của một tập hợp $S$ là một tập hợp với các phần tử là tất cả các tập hợp con của $S$. Chúng ta kí hiệu tập hợp lũy thừa của $S$ là $\mathcal{P}(S)$.
\end{definition}
...
Trong nhiều tài liệu khác, các tác giả còn dùng kí hiệu $\subset$ để chỉ quan hệ là tập hợp con, và dùng kí hiệu $\subsetneq$ để chỉ quan hệ là tập hợp con thực sự. Còn ở tài liệu này, chúng tôi quy ước dùng kí hiệu $\subseteq$ để chỉ quan hệ là tập hợp con, và dùng kí hiệu $\subset$ để chỉ quan hệ là tập hợp con thực sự, gợi sự tương tự với cặp kí hiệu $\leq$ và $<$.
...
\begin{exercise}\label{naive-set-theory:exercise3}
	Cho tập hợp $S$ gồm tất cả các chuỗi nhị phân có độ dài $n$. $S$ có bao nhiêu phần tử?
\end{exercise}

\begin{exercise}\label{naive-set-theory:exercise4}
	Cho các tập hợp $A, B, C$. Chứng minh rằng nếu $A\subseteq B$, $B\subseteq C$ thì $A\subseteq C$.

	Chứng minh rằng nếu $A\subseteq B$ và $B\subset C$, hoặc $A\subset B$ và $B\subseteq C$, hoặc $A\subset B$ và $B\subset C$, thì $A\subset C$.
\end{exercise}
...
Mục này giới thiệu khái niệm vị từ\index{Vị từ}. Chúng ta bắt đầu bằng hai ví dụ thực tế.
...
\begin{enumerate}[label={(\roman*)}]
	\item Có ít nhất một phần tử $x$ của $S$ sao cho có $p(x)$. Khi đó chúng ta kí hiệu
				\[
					\exists x\in S\; p(x)\qquad\text{hay}\qquad \exists x\in S\; (p(x))\qquad\text{hay}\qquad \exists x\in S: p(x)
				\]
	...
	\item Mọi phần tử $x$ của $S$ đều làm cho $p(x)$ đúng. Khi đó chúng ta kí hiệu
				\[
					\forall x\in S\; p(x)\qquad\text{hay}\qquad \forall x\in S\; (p(x))\qquad\text{hay}\qquad \forall x\in S: p(x)
				\]
...
\begin{theorem}[Nguyên lý quy nạp toán học\index{Nguyên lý quy nạp toán học}]
...
\begin{enumerate}[label={\textbf{Bước \arabic*.}},itemindent=1cm]
	\item (Bước cơ sở\index{Bước cơ sở}) Chúng ta chứng minh có $p(0)$.
	\item (Bước quy nạp\index{Bước quy nạp}) Chúng ta chứng minh rằng với mỗi $k\geq 1$, $p(k)$ kéo theo $p(k+1)$. Ở bước này, mệnh đề $p(k)$ được gọi là \textit{giả thiết quy nạp}.
...
Chúng ta đưa ra ví dụ về chứng minh bằng phương pháp quy nạp toán học.
...
\begin{definition}[Hợp của hai tập hợp\index{Phép hợp hai tập hợp}]
...
\begin{definition}[Giao của hai tập hợp\index{Phép giao hai tập hợp}]
...
\begin{definition}[Phần bù\index{Phần bù}]
...
\begin{definition}[Hiệu của hai tập hợp\index{Hiệu của hai tập hợp}]
...
\begin{definition}[Hợp và giao của các (nhiều tùy ý) tập hợp]
	Cho trước một tập hợp $I$ khác tập hợp rỗng. ${(A_{i})}_{i\in I}$ là một họ các tập hợp\index{Họ các tập hợp}.
    
    Hợp\index{Hợp của một họ tập hợp} của \textit{họ các tập hợp $A_{i}$}, trong đó $i\in I$, là tập hợp gồm tất cả các phần tử của các tập hợp $A_{i}$, với $i\in I$. Nói cách khác, tập hợp này gồm các phần tử sao cho phần tử đó thuộc $A_{i}$ với $i\in I$ nào đó. Chúng ta kí hiệu
	\[
		\bigcup_{i\in I}A_{i} = \{ x \mid \exists i\in I: x\in A_{i} \}.
	\]

	Giao\index{Giao của một họ tập hợp} của \textit{họ các tập hợp $A_{i}$}, trong đó $i\in I$, là tập hợp gồm tất cả các phần tử đồng thời thuộc tất cả các tập hợp $A_{i}$, với $i\in I$. Chúng ta kí hiệu
	\[
...
\begin{theorem}[Công thức De Morgan\index{Công thức De Morgan}]\label{theorem:de-morgan-formula}
...
\begin{definition}[Hai tập hợp rời nhau\index{Hai tập hợp rời nhau}]
...
\begin{definition}[Phân hoạch của tập hợp khác rỗng\index{Phân hoạch của tập hợp khác rỗng}]
	Cho tập hợp $S$ khác tập hợp rỗng. Một họ tập hợp ${(A_{i})}_{i\in I}$ được gọi là một phân hoạch của tập hợp $S$ nếu và chỉ nếu các tập hợp của họ trên rời nhau từng đôi một và hợp thành của tất cả các tập hợp này là $S$. Bằng kí hiệu, chúng ta viết
	\[
...
Khi đó, chúng ta còn nói $S$ là \textit{hợp rời\index{Hợp rời}} của họ tập hợp $A_{i}$ với $i\in I$. Hãy xem xét một số ví dụ về phân hoạch.
\begin{itemize}
...
\begin{exercise}\label{set-operations:exercise4}
	Công thức De Morgan\index{Công thức De Morgan} có thể được viết dưới dạng khác, sử dụng phép toán hiệu hai tập hợp thay vì lấy phần bù. Cụ thể là
	\begin{equation*}
		\begin{split}
